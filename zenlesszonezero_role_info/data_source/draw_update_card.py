...
from ..utils.card_utils import avatar_path, bg_path, get_font, other_path, weapon_path
...
async def draw_role_pic(uid: str, role_dict: dict | list, player_info):
    ...
    bg = Image.new("RGBA", ((56 + 187 * row_num) * multiple, (380 + ((len(role_list) + row_num - 1) // row_num - 1) * 240) * multiple), (240, 236, 227, 0))
    bg_draw = ImageDraw.Draw(bg)
    draw_center_text(
        bg_draw,
        f"{nickname} UID: {uid} 数据和榜单信息更新成功" if len(role_list) < 9 else f"{nickname} UID: {uid} 已缓存角色信息",
        60 * multiple,
        (56 + 187 * row_num - 60) * multiple,
        28 * multiple,
        "#7f5b39",
        get_font(28 * multiple, "hywh.ttf"),
    )
    ...
    for index, role in enumerate(role_list):
        ...
        # 角色卡阴影
        card_bg_shadow = Image.new("RGBA", (card_size[0] + 1 * multiple, card_size[1] + 1 * multiple), (80, 78, 91, 0))
        ...
        # 角色卡内部角色名
        draw_center_text(card_bg_draw, f"{role}Lv.{data['等级']}", 36 * multiple, (36 + 85) * multiple, (5 if top_name else 164) * multiple, "white", get_font(19 * multiple, "优设标题黑.ttf"))
        ...
        if data.get("评分", "") != "":
            ...
            card_bg_draw.rounded_rectangle(
                (90, card_size[1] + 1 * multiple - 60, 220, card_size[1] + 1 * multiple - 20),
                10,
                fill=(101, 101, 101)
                if score <= 104.8
                else (86, 190, 191)
            ...
            draw_center_text(card_bg_draw, f"{score}", 36 * multiple, (36 + 85) * multiple, (164) * multiple, "white", get_font(19 * multiple, "优设标题黑.ttf"))
        # 命座
        card_bg_draw.rounded_rectangle(
            (0, card_size[1] + 1 * multiple - 80, 60, card_size[1] + 1 * multiple),
            10,
            fill=(101, 101, 101)
            if data["影画"] == 0
            else (86, 190, 191)
        ...
        draw_center_text(card_bg_draw, f"{data['影画']}", 0, 60, card_size[1] + 1 * multiple - 80, "white", get_font(30 * multiple, "优设标题黑.ttf"))

        # 武器
        if data.get("武器", {}).get("星级", "") != "":
            weapon_bg = load_image(f"{other_path}/star{data['武器']['星级'] + 1}.png", size=(60, 60))
            ...
            weapon_icon = await get_img(url=resource_url.format("ui/zzz/" + data["武器"]["图标"] + ".png"), size=(60, 60), save_path=weapon_icon, mode="RGBA")
            card_bg.alpha_composite(weapon_icon, (card_size[0] + 1 * multiple - 60, card_size[1] + 1 * multiple - 140))

            card_bg_draw.rounded_rectangle(
                (card_size[0] + 1 * multiple - 60, card_size[1] + 1 * multiple - 80, card_size[0] + 1 * multiple, card_size[1] + 1 * multiple),
                10,
                fill=(86, 190, 191)
                if data["武器"]["精炼等级"] == 1
                else (45, 158, 98)
            ...
            draw_center_text(
                card_bg_draw, f"{data['武器']['精炼等级']}", card_size[0] + 1 * multiple - 60, card_size[0] + 1 * multiple, card_size[1] + 1 * multiple - 80, "white", get_font(30 * multiple, "优设标题黑.ttf")
            )
    ...
    draw_center_text(
        bg_draw,
        f"数据时间:{(player_info.get_roles_info(role))['更新时间']}   ※ 数据更新有5分钟延迟",
        226 * multiple,
        (56 + 187 * row_num - 226) * multiple,
        bg.height - 47 * multiple,
        "#7f5b39",
        get_font(25 * multiple, "hywh.ttf"),
    )
