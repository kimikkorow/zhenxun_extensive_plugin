...
class DamageAttributes:
    ...
    defense_pct: float = 0
    mastery_pct: float = 0
    recharge_pct: float = 0
    cpct_pct: float = 0
    cdmg_pct: float = 0
    dmg_pct: float = 0
    phy_pct: float = 0
    heal_pct: float = 0
    enemy_damage_pct: float = 0
    coloring_dmg_pct: float = 0
    stance_pct: float = 0
    eff_pct_pct: float = 0
    eff_def_pct: float = 0
    joy_pct: float = 0
    ...
    mastery_inc: float = 0
    # Star Rail panel and rule fields.  Values in the public calculator use
    # percentage points, matching Miao's SR AttrItem representation.
    speed: float = 0
    base_speed: float = 0
    speed_pct: float = 0
    stance: float = 0
    ...
    eff_def: float = 0
    joy: float = 0
    super_break_ignore: float = 0
    resistance_pen: float = 0
    static_speed: float = 0
    static_stance: float = 0
    static_cpct: float = 0
    static_cdmg: float = 0
    static_heal: float = 0
    static_eff_pct: float = 0
    static_eff_def: float = 0
    static_joy: float = 0
    sp: float = 0
    kx: float = 0
    fykx: float = 0
    ...
    merrymakes: float = 0
    punchline: float = 0
    static_attr: dict[str, Any] = field(default_factory=dict)
    ...
    def direct_base(self, key: str) -> float:
        """Return the AttrItem base used by non-attack direct attributes."""
        static_name = {
            "cpct": "static_cpct",
            "cdmg": "static_cdmg",
            "dmg": "static_dmg",
            "phy": "static_phy",
            "heal": "static_heal",
            "enemy_damage": "static_enemy_damage",
            "coloring_dmg": "static_coloring_dmg",
            "stance": "static_stance",
            "eff_pct": "static_eff_pct",
            "eff_def": "static_eff_def",
            "joy": "static_joy",
        }.get(key)
        if static_name is not None:
            return getattr(self, static_name, 0)
        if key == "shield":
            return 100
        return 0

    def direct_plus(self, key: str) -> float:
        return getattr(self, key, 0) - self.direct_base(key)

    def direct_pct(self, key: str) -> float:
        return getattr(self, f"{key}_pct", 0)

    def direct_value(self, key: str) -> float:
        base = self.direct_base(key)
        return base + self.direct_plus(key) + base * self.direct_pct(key) / 100
...
class DamageResult:
    ...
    def dmg(self, value: float) -> None:
        # Miao's result objects are mutable and a few multi-hit rules add to
        # ``result.dmg`` after the initial calculation.
        self.crit = value
