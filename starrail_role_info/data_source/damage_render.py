...
BUFF_LINE_HEIGHT = 46
BUFF_ROW_PADDING = 14
BUFF_ACCENT_COLOR = (235, 216, 168)
BUFF_HINT_COLOR = (190, 194, 202)
...
def _damage_label_width(
    draw: ImageDraw.ImageDraw,
    damage_items: list[tuple[str, tuple[str, ...]]],
) -> int:
    label_font = _font(BASE_FONT_SIZE, "hywh.ttf")
    max_width = max(
        (
            draw.textlength(description, font=label_font)
            for description, _ in damage_items
        ),
        default=0,
    )
    return min(MAX_LABEL_WIDTH, max(MIN_LABEL_WIDTH, int(max_width + 28)))
...
def draw_dmg_pic(dmg: dict[str, tuple[str, ...]]) -> Image.Image:
    ...
    label_width = _damage_label_width(measure, damage_items)
    ...
    draw.line(
        (label_width, 0, label_width, damage_bottom),
        (255, 255, 255, 75),
        2,
    )
    draw.line(
        (value_split, 0, value_split, damage_bottom),
        (255, 255, 255, 75),
        2,
    )
        ...
        for source, effect_lines, source_font, row_height in buff_rows:
            ...
            for offset, line in enumerate(effect_lines):
                draw.text(
                    (
                        BUFF_LABEL_WIDTH + 16,
                        text_top + offset * BUFF_LINE_HEIGHT,
                    ),
                    line,
                    fill="white",
                    font=buff_font,
