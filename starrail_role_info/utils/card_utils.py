...
class PlayerInfo:
        ...
        try:
            ...
            if data.get("equipment", ""):
                ...
                if weapon_prop[weapon_data["tid"]]["properties"][weapon_info["精炼等级"] - 1]:
                    ...
                    if weapon_prop_info[0]["type"] != "AllDamageTypeAddedRatio":
                        weapon_prop_info = {
                            "属性名": trans_data["property"][weapon_prop_info[0]["type"]],
            ...
            for item in data.get("skillTreeList"):
                if skill_prop := role_skill.get(str(item["pointId"]))["levels"][0]["properties"]:
                    skill_prop = {
                        "属性名": trans_data["property"][skill_prop[0]["type"]],
                ...
                for relic_info in relic_list:
                    ...
                    artifact_info["主属性"] = {
                        "属性名": trans_data["property"][
                            relic_main_value[relic_item["main_affix_id"]]["affixes"][str(relic_info["mainAffixId"])]["property"]
                        ],
                        "属性值": self.get_main_relic_value(
                            relic_main_value[relic_item["main_affix_id"]]["id"],
                            relic_info["mainAffixId"],
                            artifact_info["等级"],
                        ),
                    }
                    artifact_info["词条"] = []
                            ...
                            for i in item:
                                if i:
                                    relic_suit_prop.append(
                                        {
                                            "属性名": trans_data["property"][i["type"]],
                                            "属性值": i["value"],
                                        }
                                    )
            if suit_2:
            ...
            self.cal_prop(
                prop,
                {"属性名": "能量恢复效率", "属性值": 0},
                skill_prop_list + [weapon_prop_info] + relic_suit_prop,
            )
    ...
    def get_role_value(self, item, id, prop, level):
        if prop == 0:
            val = role_value[id]["values"][prop][item]["base"] + role_value[id]["values"][prop][item]["step"] * (level - 1)
        else:
            val = role_value[id]["values"][prop][item]["base"] + role_value[id]["values"][prop][item]["step"] * (level - 1)
        return val
    ...
    def get_weapon_value(self, item, id, prop, level):
        if prop == 0:
            val = weapon_val[id]["values"][prop][item]["base"] + weapon_val[id]["values"][prop][item]["step"] * (level - 1)
        else:
            val = weapon_val[id]["values"][prop][item]["base"] + weapon_val[id]["values"][prop][item]["step"] * (level - 1)
        return val
    ...
    def get_main_relic_value(self, id, affixes_id, level):
        val = (
            relic_main_value[id]["affixes"][str(affixes_id)]["base"] + relic_main_value[id]["affixes"][str(affixes_id)]["step"] * level
        )  # ((level if level else 1) - 1)
        return val

    def get_sub_relic_value(self, id, affixes_id, level, step_num):
        val = relic_sub_value[id]["affixes"][str(affixes_id)]["base"] * level + relic_sub_value[id]["affixes"][str(affixes_id)]["step"] * step_num
        return val
