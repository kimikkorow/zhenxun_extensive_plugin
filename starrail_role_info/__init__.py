...
__plugin_meta__ = PluginMetadata(
    ...
    查询角色的面板
    指令：
        星铁绑定uidXXXX (例:星铁绑定uid123456789)
        星铁解绑
        角色面板 (例:希儿面板、希儿面板@XXX、希儿面板123456789)
        更新/刷新星铁面板 (uid) (例:更新星铁面板123456789)
...
bind = on_regex(r"(?:星铁绑定|绑定星铁|崩铁绑定|绑定崩铁).*?(\d+)", priority=5, block=True)
...
async def check_role_avaliable(role_name, roles_list, event):
    if not roles_list:
        guide = load_image(f"{other_path}/collections.png")
        guide = image_build(img=guide, quality=100, mode="RGB")
        await his_card.finish(  # MessageSegment.reply(event.message_id) +
            guide + "无角色信息,在游戏中将角色放入漫游签证并输入更新星铁面板XXXX(uid)!",
            at_sender=False,
        )
...
async def _(event: MessageEvent, args: tuple[str, ...] = RegexGroup()):
    ...
    role_data = player_info.get_roles_info(role_name)
    img, _ = await gen_artifact_recommend(  # MessageSegment.reply(event.message_id) +
        f"{role}{msg[1]}遗器推荐",
        role_data,
        artifact_list,
        uid,
...
async def _(event: MessageEvent, args: tuple[str, ...] = RegexGroup()):
    ...
    merge_ = role + at_user
    # if role not in ["更新星铁", "刷新星铁", "更新崩铁", "刷新崩铁"]:
    if "星铁" not in merge_ and "崩铁" not in merge_:
    ...
    if at_user.isdigit():
        ...
        uid = await get_msg_uid(event)
    if ("更新" in merge_ or "刷新" in merge_) and ("星铁" in merge_ or "崩铁" in merge_):
        if at_user:
...
async def get_char(uid, event):
    ...
    if not roles_list:
        ...
        await my_card.finish(  # MessageSegment.reply(event.message_id) +
