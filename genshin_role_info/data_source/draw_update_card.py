...
async def draw_role_pic(uid: str, role_dict: dict | list, player_info):
    ...
    draw_center_text(
        bg_draw,
        f"{nickname} UID: {uid} 数据和榜单信息更新成功"
        if len(role_list) < 9
        else f"{nickname} UID: {uid} 已缓存角色信息",
        60 * multiple,
    ...
    top_line = Image.new(
        "RGBA",
        ((56 + 187 * row_num - 100) * multiple, 5 * multiple),
        (224, 217, 207, 0),
    )
    ...
    for index, role in enumerate(role_list):
        ...
        card_bg_shadow = Image.new(
            "RGBA",
            (card_size[0] + 1 * multiple, card_size[1] + 1 * multiple),
            (80, 78, 91, 0),
        )
        ...
        card_bg.alpha_composite(
            bg_img.resize((card_bg.width, card_bg.height), Image.Resampling.LANCZOS),
            (0, 0),
        )
        card_bg_draw = ImageDraw.Draw(card_bg)

        # 角色图
        role_bg = f"{avatar_path}/{('UI_AvatarIcon_Side_' + role_info_json[role]['英文名']).replace('_Side', '')}.png"
        role_bg = await get_img(
            url=role_url.format(
                ("UI_AvatarIcon_Side_" + role_info_json[role]["英文名"]).replace(
                    "_Side", ""
                )
            ),
            save_path=role_bg,
        ...
        draw_center_text_with_safe_advance(
            ...
            36 * multiple,
            (36 + 85) * multiple,
            (5 if top_name else 164) * multiple,
            "white",
        ...
        if data.get("评分") not in (None, ""):
            ...
            card_bg_draw.rounded_rectangle(
                (
                    90,
                    card_size[1] + 1 * multiple - 60,
                    220,
                    card_size[1] + 1 * multiple - 20,
                ),
                10,
                fill=(101, 101, 101)
                if score < 14 * 5
        ...
        weapon_bg = load_image(
            f"{other_path}/star{data['武器']['星级']}.png", size=(60, 60)
        )
        card_bg.alpha_composite(
            weapon_bg,
            (card_size[0] + 1 * multiple - 60, card_size[1] + 1 * multiple - 140),
        )
        ...
        card_bg_draw.rounded_rectangle(
            (
                card_size[0] + 1 * multiple - 60,
                card_size[1] + 1 * multiple - 80,
                card_size[0] + 1 * multiple,
                card_size[1] + 1 * multiple,
            ),
            10,
            fill=(86, 190, 191)
            if data["武器"]["精炼等级"] == 1
